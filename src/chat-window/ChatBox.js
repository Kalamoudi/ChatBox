...
//   }
  ...
  const handleMessageView = (messages) => {
    ...
    let widthUnit = 1
    let heightUnit = 1
    const extraSpace = letterWidth*10
    ...
    messages.forEach((message, index) => {
        ...
        let classN = message.sender==="me" ? msgStyle : msgOtherStyle
        ...
        htmlElements.push(
            <div key={index} style={{...combinedDic, position: 'relative'}}>
                ...
                <p style={{ color: 'grey', fontSize: '12px', position: 'absolute', top: 3, right: 24 }}>{currentTime}</p>
            </div>
        )
  ...
  const handleSenderChange = () => {
    setSender(prevSender => prevSender === 'me' ? 'other' : 'me');
  };
